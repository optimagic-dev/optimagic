...
Args:
    noise_handling: Method for handling the noise. The name can be (Default: `None`).
        - "random": A random point is reevaluated regularly using the one-fifth
          adaptation rule.
        - "optimistic": The best optimistic point is reevaluated regularly, embracing
        ...
        - A float coefficient can be provided to tune the regularity of these
          ...
          chance (i.e., 1 in 20) of being repeated (i.e., the same candidate solution
          is reevaluated to better estimate its performance).
    ...
    mutation: Type of mutation to apply. Available options are (Default: `"gaussian"`).
        - "gaussian": Standard mutation by adding a Gaussian random variable (with
          progressive widening) to the best pessimistic point.
    ...
    sparse: Whether to apply random mutations that set variables to zero. Default is
        `False`.

    smoother: Whether to suggest smooth mutations. Default is `False`.

    annealing: Annealing schedule to apply to mutation amplitude or temperature-based
        control. Options are:
        - "none": No annealing is applied.
        - "Exp0.9": Exponential decay with rate 0.9.
        - "Exp0.99": Exponential decay with rate 0.99.
        - "Exp0.9Auto": Exponential decay with rate 0.9, auto-scaled based on problem
          horizon.
        - "Lin100.0": Linear decay from 1 to 0 over 100 iterations.
        - "Lin1.0": Linear decay from 1 to 0 over 1 iteration.
        - "LinAuto": Linearly decaying annealing automatically scaled to the problem
          horizon. Default is `"none"`.

    super_radii: Whether to apply extended radii beyond standard bounds for candidate
        generation, enabling broader exploration. Default is `False`.

    roulette_size: Size of the roulette wheel used for selection in the evolutionary
        process. Affects the sampling diversity from past candidates. (Default: `64`)

    antismooth: Degree of anti-smoothing applied to prevent premature convergence in
        smooth landscapes. This alters the landscape by penalizing overly smooth
        improvements. (Default: `4`)
        
    crossover: Whether to include a genetic crossover step every other iteration.
        Default is `False`.

    crossover_type: Method used for genetic crossover between individuals in the
        population. Available options (Default: `"none"`):
        ...
        - "onepoint": One-point crossover, splitting the genome at a single random
          point.
        - "twopoint": Two-point crossover, splitting the genome at two points and
          exchanging the middle section.

    tabu_length: Length of the tabu list used to prevent revisiting recently evaluated
        candidates in local search strategies. Helps in escaping local minima.
        (Default: `1000`)

    rotation: Whether to apply rotational transformations to the search space,
        promoting invariance to axis-aligned structures and enhancing search
        performance in rotated coordinate systems. (Default: `False`)
        """

    noise_handling: Literal["random", "optimistic"] | tuple[
        Literal["random", "optimistic"], float] | None = None
    mutation: Literal[
        "gaussian",
        "cauchy",
        "discrete",
