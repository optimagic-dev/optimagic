...
def get_components(constraints, n_constr):  # WiP
    ...
    if len(output) != n_constr:
        raise ValueError(
            f"The number of constraints does not match. Expected {n_constr}, got {len(output)}"
        )
...
constraints_all = [
    {"type": "nonlinear", "func": constraint_func_2d, "lower_bound": np.arange(2)},
    {"type": "nonlinear", "func": constraint_func_1d, "value": 2},
]

constraints_1d = [{"type": "nonlinear", "func": constraint_func_1d, "value": 2}]
constraints_2d = [
    {"type": "nonlinear", "func": constraint_func_2d, "lower_bound": np.arange(2)}
]
...
def test_get_components_all():
    ...
    expected_output = [
        {"type": "nonlinear", "func": constraint_func_2d(x)[0], "lower_bound": 0},
        {"type": "nonlinear", "func": constraint_func_2d(x)[1], "lower_bound": 1},
        {"type": "nonlinear", "func": constraint_func_1d, "value": 2},
    ]
