...
def minimize_gqtpar_quadratic_fast(model, *, k_easy=0.1, k_hard=0.2, maxiter=200):
    ...
    gradient_norm = np.linalg.norm(model.linear_terms)
    lambdas = get_initial_guess_for_lambdas_fast(model)
    ...
    for _niter in range(maxiter):
        ...
        if hessian_info.already_factorized:
            ...
            hessian_info, factorization_info = add_lambda_and_factorize_hessian_fast(
                model, hessian_info, lambdas
            )
        ...
        if factorization_info == 0 and gradient_norm > zero_threshold:
            ...
            lambdas = update_lambdas_when_factorization_unsuccessful_fast(
                hessian_info,
                lambdas,
                factorization_info,
            )
