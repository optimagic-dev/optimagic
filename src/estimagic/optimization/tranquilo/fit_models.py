...
def get_fitter(fitter, user_options=None, model_info=None):
    ...
    built_in_fitters = {
        ...
        "_pounders_original": fit_pounders_original,
...
def fit_flexible(x, y, model_info):
    ...
    if n_samples <= n_params + 1:
        ...
        coef = fit_pounders_original(x, y, model_info)
    else:
...
def fit_pounders_experimental(x, y, model_info):
    ...
    coef = _get_current_fit_pounders_experimental(
        y,
        m_mat,
        n_mat,
        z_mat,
...
def _get_current_fit_pounders_original(
    ...
    for k in range(n_residuals):
        ...
        coeffs_square[k] = _transform_interactions_and_square_terms_back(
            coeffs_second_stage, n_params, has_squares
        )
...
def _get_current_fit_pounders_experimental(
    y,
    m_mat,
    n_mat,
    z_mat,
...
def _get_feature_matrices_pounders_original(x, model_info):
    ...
    for i in range(n_samples):
        m_mat[i, 1:] = x[i]
        n_mat[i, :] = _interactions_and_square_features_original_pounders(
            m_mat[i, 1:], has_squares
        )
...
@njit
def _interactions_and_square_features_original_pounders(x, has_squares):
    x = np.atleast_2d(x)
    n_samples, n_params = x.shape
    ...
    if has_squares:
        ...
        j = 0
        for i in range(n_params):
            poly_terms[j] = 0.5 * xt[i] ** 2
            j += 1

            for k in range(i + 1, n_params):
                poly_terms[j] = xt[i] * xt[k] / np.sqrt(2)
                j += 1
...
@njit
def _transform_interactions_and_square_terms_back(poly_terms, n_params, has_squares):
    if has_squares:
        idx = 0
        for i in range(n_params):
            poly_terms[idx] *= 0.5
            ...
            for _ in range(i + 1, n_params):
                poly_terms[idx] /= np.sqrt(2)
                idx += 1
        ...
        for i in range(n_params):
            j_start = i if has_squares else i + 1
            for _ in range(j_start, n_params):
                poly_terms[idx] /= np.sqrt(2)
                idx += 1
