...
def get_sampler(sampler, bounds, model_info=None, user_options=None):
    ...
    built_in_samplers = {
        "random_filled": _random_interior_sampler,
        "random_hull": _random_hull_sampler,
...
def _random_interior_sampler(
    ...
    # Assign sampler dynamically
    if trustregion.shape == "sphere":
        _sampler = _sphere_sampler
    elif trustregion.shape == "cube":
        _sampler = _cube_sampler
    else:
        raise ValueError("Trustregion shape must be in {'sphere', 'cube'}.")

    # Compute intermediate data and effective bounds
...
def _cube_sampler(
    n_params, n_points, rng, effective_bounds, existing_xs=None  # noqa: ARG001
):
...
def _sphere_sampler(
    ...
    scaled = raw * scale.reshape(-1, 1)
    points = _map_into_feasible_trustregion(scaled, bounds=effective_bounds)
    return points


def _random_hull_sampler(
    trustregion,
    n_points,
    rng,
    order,
