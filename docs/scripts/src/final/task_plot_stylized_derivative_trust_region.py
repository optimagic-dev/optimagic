...
def plot_derivative_based_trust_region(textstr, save_path):
    path = 'docs/source/_static/images/intermediate_result/derivative_based_trust_region'
    ...
    for i in range(5):
        if i < 3:
            ...
            radius=start_radius
            start_radius-=1
        fig, ax, x = tools.plot_trust_region_algo(x, radius, surrogate_func=tools.regression_surrogate)
        fig.set_size_inches(8, 6)
        plt.subplots_adjust(bottom=0.2)
        plt.figtext(0.5, -0.01, textstr[i],multialignment='center', ha="center", fontsize=18, bbox={"facecolor":"white", "alpha":0.05, "pad":5})
        x=x
...
@pytask.mark.produces(BLD/"images"/"final_result"/"derivative_based_trust_region_algorithm.gif")
