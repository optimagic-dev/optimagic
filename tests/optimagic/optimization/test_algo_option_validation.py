"""Test validation and conversion of algo options.

This module must mirror test_algo_option_validation_stringified.py, which contains
the same tests for a module with postponed annotation evaluation.
...
def test_field_types_are_type_objects():
    # Guard: if this fails, the module no longer covers the code path for regular
    # annotations and the mirrored test setup must be adapted.
